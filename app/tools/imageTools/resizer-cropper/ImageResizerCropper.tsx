...
export default function ImageResizerCropper() {
  ...
  useEffect(() => {
    ...
    img.onload = () => {
      ...
      ctx.save();
      ctx.translate((imgW * scale) / 2, (imgH * scale) / 2);
      ...
      switch(cropShape){
        case 'rectangle':
          ctx.fillRect(cropPos.x, cropPos.y, cropSize.w, cropSize.h);
          ctx.strokeRect(cropPos.x, cropPos.y, cropSize.w, cropSize.h);
          break;
        case 'circle':
          ctx.beginPath();
          ctx.arc(cropPos.x + cropSize.w/2, cropPos.y + cropSize.h/2, cropSize.w/2, 0, Math.PI*2);
          ctx.fill();
          ctx.stroke();
          break;
        case 'triangle':
          ctx.beginPath();
          ctx.moveTo(cropPos.x + cropSize.w/2, cropPos.y);
          ctx.lineTo(cropPos.x, cropPos.y + cropSize.h);
          ctx.lineTo(cropPos.x + cropSize.w, cropPos.y + cropSize.h);
          ctx.closePath();
          ctx.fill();
          ctx.stroke();
          break;
  ...
  const handleDownload = () => {
    ...
    if (cropShape === 'rectangle') {
      ...
      if(cropShape==='circle') ctx.arc(cropSize.w/2, cropSize.h/2, cropSize.w/2, 0, Math.PI*2);
        ...
        ctx.moveTo(cropSize.w/2,0);
        ctx.lineTo(0,cropSize.h);
        ctx.lineTo(cropSize.w,cropSize.h);
        ctx.closePath();
      }
      ctx.clip();
      ctx.drawImage(canvas, cropPos.x, cropPos.y, cropSize.w, cropSize.h, 0,0,cropSize.w,cropSize.h);
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-linear-to-br from-gray-100 to-gray-200 p-6">
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-4xl bg-white rounded-2xl shadow-lg p-6">
        <h1 className="text-2xl font-semibold mb-4">Image Resizer & Cropper – Resize and Crop Images Online</h1>
